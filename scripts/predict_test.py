...
path = '../datasets/test/'
model_name = 'data0_Cascaded_efb5_loss_augmentation_8_adversarial_nadam_different_AUG'

ensemble = 1
model_name2 = 'data0_Cascaded_efb3_loss_augmentation_8_adversarial'
...
if ensemble == 1:
	...
	Ydata2 = model2.predict(x=Xdata, batch_size=16, verbose=1)
...
if ensemble == 1:
	output_path = '../logs/{}/ensemble_with_{}/'.format(model_name, model_name2)
	create_directory(output_path)
...
for pid in range(number_of_samples):
	...
	if ensemble == 1:
		temp2 = Ydata2[pid, :, :, :]
		temp_dimension = dimensions[pid]
		temp2 = cv2.resize(temp2, (int(temp_dimension[1]), int(temp_dimension[0])), interpolation=cv2.INTER_LINEAR)

		temp = np.logical_or(temp>=0.5, temp2>=0.5)
