...
CFG_NAME = "unet_efb3_diceloss_full" # name of the architecture/configuration for segmentation model
...
total_loss = (1 * focal_loss) + (1 * jacard_loss)
...
if (unet_or_srunet ==0 or unet_or_srunet == 1):
    ...
    if augmentation_flag == 1:
        #generators
        training_generator = classes.DataGenerator_Augment(train_data, train_labels, batch_size=batch_size, shuffle=True) # x_train,y_train
        #validation_generator = classes.DataGenerator_Augment(x_test, y_test, batch_size=batch_size, shuffle=True)

        #Train model on dataset
        model.fit_generator(generator=training_generator,
                            validation_data=None,callbacks=[csv_logger], #(x_test, y_test), checkpointer, reduce_lr, early_stopping
                        shuffle=True,
                        verbose = 2,epochs = 50, steps_per_epoch= (len(x_train)*2) // batch_size) #epochs
